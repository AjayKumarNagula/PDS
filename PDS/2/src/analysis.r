...
Student = read.csv("C:\\Users\\ajayn\\OneDrive\\Ajay\\PDS\\2\\data_clean\\clean_StudentsData.csv")
...
plot(Student_Data$gender, Student_Data$reading.score,
     xlab = "SEX", ylab = "READING SCORE",main="BOXPLOTS OF READING SCORE VS GENDER",col.main="blue", col=(c("yellow","violet")))

#2.3
Student_Data$gender <- as.factor(Student_Data$gender)
plot(Student_Data$gender, Student_Data$writing.score,
     xlab = "SEX", ylab = "WRITING SCORE",main="BOXPLOTS OF WRITING SCORE VS GENDER",col.main="yellow", col=(c("navyblue","purple")))
...
ggplot(data = Student_Data, aes(x = math.score, y = reading.score, col = gender)) +
  ...
  scale_color_manual(values = c("male" = "red", "female" = "orange")) +
